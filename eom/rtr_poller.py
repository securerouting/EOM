...
class RtrRIBState:
    ...
    def store_in_db(self, rib, ts):
        ...
        Args:
            ...
            ts: Timestamp associated with the polled data

        Returns:
            None
        """
        if not rib:
            print "Could not refresh database with new data for %s." % self.device
        else:
            ...
            try:
                ...
                cur.execute("INSERT INTO rtr_cache (device, rtrupdt) VALUES (?, ?)", (self.device, ts))
    ...
    def update(self, output, ts):
        """Parse output and store results to database.

        Args:
            output: The output generated by do_poll.py
            ts: Timestamp associated with the poll operation

        Returns:
            A dict containing parsed RIB entries
        """
        ribinfo = self.parse(output)
        self.store_in_db(ribinfo, ts)
...
class EOMRtrRIBFetcher(EOMGenericPoller):
    ...
    def poll(self, now):
        ...
        if now - self.last_update > self.poll_interval:
            ...
            if self.realm:
                output = subprocess.check_output(["do_poll.py", "--device", self.device, "--realm", self.realm])
            else:
                output = subprocess.check_output(["do_poll.py", "--device", self.device])
            ...
            self.ribstate.update(output, now)
