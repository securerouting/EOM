...
"""
Iterator class to parse the output of an rcynic run.
"""

__version__ = '$Id: rcynic.py 5845 2014-05-29 22:31:15Z sra $'
...
class rcynic_certificate(rcynic_object):
  ...
  def __init__(self, filename, **kwargs):
    ...
    self.sia_directory_uri, self.manifest_uri, self.object_uri  = self.obj.get_SIA()
    ...
    self.crldp = self.obj.get_POW().getCRLDP()
...
class rcynic_manifest(rcynic_object):
  obj_class = rpki.x509.SignedManifest

  def __init__(self, *args, **kwargs):
    rcynic_object.__init__(self, *args, **kwargs)
    self.obj.extract()
    self.thisUpdate = self.obj.getThisUpdate()
    self.nextUpdate = self.obj.getNextUpdate()
    self.manifestNumber = self.obj.get_POW().getManifestNumber()
    self.ee = rpki.x509.X509(POW = self.obj.get_POW().certs()[0])
    self.notBefore = self.ee.getNotBefore()
    self.notAfter = self.ee.getNotAfter()
    self.aia_uri = self.ee.get_aia_uri()
    ...
    self.aki = self.ee.hAKI()
    self.ski = self.ee.hSKI()
    self.files = self.obj.get_POW().getFiles()
    self.algorithm = self.obj.get_POW().getAlgorithm()


class rcynic_crl(rcynic_object):
  obj_class = rpki.x509.CRL

  def __init__(self, *args, **kwargs):
    rcynic_object.__init__(self, *args, **kwargs)
    self.thisUpdate = self.obj.getThisUpdate()
    self.nextUpdate = self.obj.getNextUpdate()
    self.crlNumber = self.obj.getCRLNumber()
    self.issuer = self.obj.getIssuer()
    self.aki = self.obj.hAKI()
    try:
        self.revoked = self.obj.get_POW().getRevoked() # list of (serial,date) pairs
    except rpki.POW.POWError:
        self.revoked = []
...
class rcynic_xml_iterator(object):
  ...
  def __iter__(self):
    for validation_status in ElementTree(file=self.xml_file).getroot().getiterator("validation_status"):
      ...
      kw = dict(timestamp = timestamp, generation = generation, uri=uri, status = status)
      if ext in file_name_classes:
	kw.update(filename = filename, file_class = file_name_classes[ext])
      yield validation_status_element(**kw)
