...
class ReadBuffer(object):
  ...
  def check_version(self, version):
    ...
    if self.version is not None and version != self.version:
      raise eom.rpki.rtr.pdus.CorruptData(
        "Received PDU version %d, expected %d" % (version, self.version))
    if self.version is None and version not in eom.rpki.rtr.pdus.PDU.version_map:
      raise eom.rpki.rtr.pdus.UnsupportedProtocolVersion(
        "Received PDU version %s, known versions %s" % (
          version, ", ".join(str(v) for v in eom.rpki.rtr.pdus.PDU.version_map)))
...
class PDUChannel(asynchat.async_chat, object):
  ...
  def __init__(self, root_pdu_class, sock = None):
    ...
    assert issubclass(root_pdu_class, eom.rpki.rtr.pdus.PDU)
  ...
  def start_new_pdu(self):
    ...
    try:
      ...
      while p is not None:
        self.deliver_pdu(p)
        p = self.root_pdu_class.read_pdu(self.reader)
    except eom.rpki.rtr.pdus.PDUException, e:
